...
class Command(commandutil.TrackerCommand):
    ...
    def build_search_url(self, name):
        ...
        return searchUrlBase.format(**dict(key=self.apiKey, game=urllib.parse.quote(name), limit=self.queryLimit))
    ...
    def parse_query_results(self, searchResult):
        ...
        return dict(
            ...
            platforms=list([x['abbreviation'] for x in searchResult['platforms'] or []])
        )
    ...
    def process_query(self, run, searchResult):
        ...
        if run.name != parsed['name']:
            ...
            if self.compiledCleaningExpression.search(run.name):
                self.message('Detected run name {0} (id={1}) may have category information embedded in it.'.format(run.name, run.id), 0 if self.interactive else 1)
                if self.interactive:
                    self.message('Please set a category for this run (hit enter to leave as {0})'.format(run.category), 0)
                    input = input(' -> ')
                    if input != '':
                        run.category = input
        ...
        if parsed['release_year'] == None:
            if self.interactive:
                ...
                while not isinstance(val, int):
                    self.message("Enter the release year (leave blank to leave as is): ", 0)
                    input = input(' -> ')
                    if input == '':
                        break
                ...
                self.message("No release date info found for {0} (id={1}), you will need to fix this manually.".format(run.name, run.id))
        elif run.release_year != parsed['release_year']:
            self.message("Setting run {0} release_year to {1}".format(run.name, parsed['release_year']), 2)
            run.release_year = parsed['release_year']

        platformCount = len(parsed['platforms'])
        if run.console in parsed['platforms']:
            self.message("Console already set for {0} to {1}.".format(run.name, run.console), 0)
            ...
            if self.interactive:
                val = None
                if platformCount == 0:
                    self.message("Select a console, or enter a name manually (leave blank to keep as is):")
                else:
                    ...
                    self.message("Multiple platforms found for {0}, leaving as is for now.".format(run.name), 0)
        else:
    ...
    def process_search(self, run, cleanedRunName, searchResults):
        ...
        if len(potentialMatches) == 0:
            ...
            if self.interactive:
                self.message("Multiple matches found for {0}, please select one:".format(cleanedRunName), 0)
                ...
                for i in range(0, numMatches):
                    ...
                    self.message("{0}) {1} ({2}) for {3}".format(i+1, parsed['name'], parsed['release_year'], ', '.join(parsed['platforms'] or ['(Unknown)'])), 0)
                val = None
                while not isinstance(val, int) or (val < 1 or val > numMatches):
                    self.message("Please select a value between 1 and {0} (enter a blank line to skip)".format(numMatches), 0)
                    input = input(' -> ')
                    if input == '':
                        val = None
                        break
                ...
                self.message("Multiple matches found for {0}, skipping for now".format(cleanedRunName))
                self.foundAmbigiousSearched = True
        else:
